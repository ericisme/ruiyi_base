...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn"%>        
                ...
                <section>
                  <div class="alert alert-info">报表详细2- <strong>指定日期(派票/投幣)(不同遊戲)</strong></div>
                </section>
                <a href="javascript:reportDetailBack();" class="btn btn-primary margin-bottom">&lsaquo;&nbsp;返回列表</a>                
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped">
                      <thead >
                        <tr id="tr_title">
                        	<th colspan="2"><strong>日期/游戏/统计项目</strong></th>
			...
			function query(){
				...
				$("#report_content").hide();
				$.ajax({
		   			url : '${ctx}/frontEnd/notDecoratorsGameCenter/report/GAME_DAILY_TX_REPORT_QUERY',
		   			type : 'post', //数据发送方式
		   			dataType : 'JSON', //接受数据格式       
		   			data: "begin_date="+begin_date+"&until_date="+until_date+"&consumer_key="+consumer_key, //要传递的数据       
		   			success :
			...
			function initReport(reportJson){
				...
				if(consumers.length>0){
					for(var i=0; i<consumers[0].data.length; i++){						
						for(var j=0; j<consumers.length; j++){
							var data = consumers[j].data[i];
							tb_dailys +="<tr>";			
								if(j==0){
									tb_dailys +="<td rowspan='"+consumers.length+"'>"+data.date+"</td>";
								}
								tb_dailys +="<td>"+consumers[j].name+"</td>"
								tb_dailys +="<td>"+data.token.count+"</td>";
								tb_dailys +="<td>"+data.token.credit+"</td>";
								tb_dailys +="<td>"+data.token.debit+"</td>";
								tb_dailys +="<td>"+data.ticket.count+"</td>";
								tb_dailys +="<td>"+data.ticket.credit+"</td>";
								tb_dailys +="<td>"+data.ticket.debit+"</td>";
							tb_dailys +="</tr>";
						}
					}
				}
