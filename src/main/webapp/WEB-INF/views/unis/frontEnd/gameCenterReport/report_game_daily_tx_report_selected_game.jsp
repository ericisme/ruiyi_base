...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn"%>        
                ...
                <section>
                  <div class="alert alert-info">报表详细- <strong>選擇遊戲(派票/投幣)(不同遊戲)</strong></div>
                </section>
                <a href="javascript:reportDetailBack();" class="btn btn-primary margin-bottom">&lsaquo;&nbsp;返回列表</a>                
                
                    <div class="margin-bottom"> <!-- 加上这个 div-->
                  			<select class="form-control" id="consumer_key" name="consumer_key" onchange="query();">
                  				<c:forEach  items="${games }" var="game" varStatus="status">
                  					<option value="${game.consumer_key }">${game.consumer_name }</option>
                  				</c:forEach>
                  			</select>
                    </div> <!-- 加上这个 div-->               
                  ...
                  <div class="animated animated_fast fadeIn" id="report_loading" style="display:none">
			...
			function query(){
				...
				$("#report_content").hide();
				$.ajax({
		   			url : '${ctx}/frontEnd/notDecoratorsGameCenter/report/GAME_DAILY_TX_REPORT_QUERY_SELECTED_GAME',
		   			type : 'post', //数据发送方式
