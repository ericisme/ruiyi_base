		...
		<c:otherwise>
			<c:forEach items="${page }" var="entity" varStatus="status">
				<c:choose>
					...
					<td title="双击查看">${entity.mtype  }</td>
					<td title="双击查看">${entity.mkey  }</td>
					<td title="双击查看">${entity.mvalue  }</td>
					<td title="双击查看">${entity.orderNum  }</td>
					<td title="双击查看"><c:if test="${entity.datelevel eq 1}">启用</c:if><c:if test="${entity.datelevel eq 0}">停用</c:if></td>			
					<td>
						...
						<shiro:hasPermission name="mcode:delete">
							<input class="button_2" name="" type="button" value="删除" onclick="del(${entity.id});"/>
					    </shiro:hasPermission>
					    <shiro:hasPermission name="mcode:setup">
					    	<input class="button_2" id="setUnable${entity.id}" type="button" value="停用" onclick="setup(${entity.id}, 0);" <c:if test="${entity.datelevel eq 0}">style="display:none;"</c:if> />
							<input class="button_2" id="setAble${entity.id}"   type="button" value="启用" onclick="setup(${entity.id}, 1);" <c:if test="${entity.datelevel eq 1}">style="display:none;"</c:if> />
						</shiro:hasPermission>
						<!--<shiro:hasPermission name="news:UpDown">
