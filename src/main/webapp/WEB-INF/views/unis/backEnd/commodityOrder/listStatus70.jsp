		...
		<c:otherwise>
			...
			<tr class="biaoge_21">
				<td colspan="9" align="left">
					...
					<br/>
					物流商：${commodityOrderGroupList.commodityOrderList[0].commodityLogistics.name } &nbsp;
					运单号：${commodityOrderGroupList.commodityOrderList[0].logisticsCode }
					</font>
				</td>
			</tr>				
					...
					<c:if test="${(status2.count eq 1)}">
						<td width="2%" rowspan="${fn:length(commodityOrderGroupList.commodityOrderList) }">
							<c:if test="${fn:length(commodityOrderGroupList.commodityOrderList) gt 1 }">
								<input type="checkbox" onclick="base.checkAll('cbx_${commodityOrderGroupList.groupLogisticsCode }',this);"/>
							</c:if>
							<c:if test="${fn:length(commodityOrderGroupList.commodityOrderList) eq 1 }">
							</c:if>
						</td>
					</c:if>		
					<td width="2%"><input type="checkbox" name="cbx_${commodityOrderGroupList.groupLogisticsCode }" value="${commodityOrder.id }"/></td>
