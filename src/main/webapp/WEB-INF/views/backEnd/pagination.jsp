...
<c:url var="firstUrl" value="${dir}/1${per}" />
<c:url var="lastUrl" value="${dir }/${page.totalPages}${per}" />
<c:url var="prevUrl" value="${dir }/${currentIndex - 1}${per}" />
<c:url var="nextUrl" value="${dir }/${currentIndex + 1}${per}" />
<input type="hidden" id="currentPageIndex" value="${currentIndex}"/>
<c:set var="total" value="${page.totalElements}" />
		
<c:set var="pre" value="${currentIndex gt 1}" scope="page"/>
<c:set var="end" value="${currentIndex lt page.totalPages}" scope="page"/>
		...
		<c:forEach var="i" begin="1" end="${page.totalPages }" step="1"> 
	     	 <c:choose>
	     	 	<c:when test="${currentIndex eq i}">
	     	 		<option value="${i }" selected>${i }/${page.totalPages }</option>
	     	 	</c:when>
	     	 	<c:otherwise>
	     	 		<option value="${i }">${i }/${page.totalPages }</option>
	     	 	</c:otherwise>
	     	 </c:choose>
	    </c:forEach>
	</select>	
