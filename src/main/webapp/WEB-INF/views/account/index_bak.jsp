	...
	<div class="weizhi" >你的位置是：系统管理 >> 用户管理</div>
		<table class="biaoge_1" style="margin-top:10px;">
			<tr>
				...
				<td class="biaoge_12">
				<c:choose>
						<c:when test="${empty dw_select }">
						<span style="display: none">				
							<div name="_xx_id" id="_xx_id"  value="${user.typeId}"></div>
						</span>
						</c:when>
						<c:otherwise>
							<select name="_dw_id" id="_dw_id" onchange="getXxSelect(this.value)">
							<option value="0">请选择</option>
								${dw_select}
							</select>
							<span id="_xx_select">
							<select name="_xx_id" id="_xx_id">
								<option value="0">请选择</option>
								${xx_select}
							</select>
							</span>
